...
public class NaverReviewsService {
	...
	public Page<NaverReviews> getList(int page) {	
		/*
		 *  기능 정리
		 *  PageRequesst -> 페이지번호, 페이지당 데이터개수, 정렬조건, ... -> SQL 생성
		 *  Pageable 	 -> SQL 실행하는 담당, 레포지토리가 SQL 실행시 관여 -> 결과물 제공
		 *  Page -> 페이지번호, 페이지당 데이터개수, 정렬조건 부합한는 결과 셋을 가지고 있다
		 *  차후 보안 이슈로 조정 -> NaverReviews는 엔티티 DTO로 대체처리 혹은 교환처리
		 */
		// 데이터 정렬은 미포함 상태
